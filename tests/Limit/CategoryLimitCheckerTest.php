<?php

namespace OrderLimitBundle\Tests\Limit;

use OrderCoreBundle\Entity\Contract;
use OrderCoreBundle\Entity\OrderProduct;
use OrderLimitBundle\Limit\CategoryLimitChecker;
use PHPUnit\Framework\Attributes\CoversClass;
use PHPUnit\Framework\Attributes\RunTestsInSeparateProcesses;
use Tourze\PHPUnitSymfonyKernelTest\AbstractIntegrationTestCase;
use Tourze\ProductCoreBundle\DTO\ProductCategory;
use Tourze\ProductCoreBundle\Entity\Sku;
use Tourze\ProductLimitRuleBundle\Entity\CategoryLimitRule;
use Tourze\ProductLimitRuleBundle\Enum\CategoryLimitType;

/**
 * CategoryLimitChecker集成测试 - 基础功能验证
 *
 * @internal
 */
#[CoversClass(CategoryLimitChecker::class)]
#[RunTestsInSeparateProcesses]
final class CategoryLimitCheckerTest extends AbstractIntegrationTestCase
{
    private CategoryLimitChecker $categoryLimitChecker;

    protected function onSetUp(): void
    {
        $this->categoryLimitChecker = self::getService(CategoryLimitChecker::class);
    }

    public function testServiceExistsShouldReturnValidInstance(): void
    {
        $this->assertInstanceOf(CategoryLimitChecker::class, $this->categoryLimitChecker);
    }

    public function testCheckCategoryLimitRuleWithValidInputsShouldExecute(): void
    {
        // 创建基本的测试数据
        $user = $this->createNormalUser();
        $this->setAuthenticatedUser($user);

        $limitRule = new CategoryLimitRule();
        $limitRule->setType(CategoryLimitType::BUY_TOTAL);
        $limitRule->setValue('10');

        $contract = new Contract();
        $contract->setUser($user);

        $sku = new Sku();
        // ID由Doctrine自动生成，不需要手动设置

        $orderProduct = new OrderProduct();
        $orderProduct->setQuantity(1);
        $orderProduct->setContract($contract);
        $orderProduct->setSku($sku);

        $category = new ProductCategory(
            id: 1,
            name: 'Test Category',
            valid: true
        );

        // 验证方法能正常执行而不抛出致命错误
        // checkCategoryLimitRule 是 void 方法，我们验证它能正常执行
        $this->expectNotToPerformAssertions();
        $this->categoryLimitChecker->checkCategoryLimitRule($limitRule, $orderProduct, $category);

        // 如果执行到这里说明没有抛出致命错误（业务异常是预期的）
    }
}
